...
class Memegen extends Component {
  ...
  handleChange = event => {
    ...
    formDataCopy[inputname] = userinput //using [] for inputname becuase it is a string 
    this.setState({
...
//     const randMemeId = this.state.allmemeImgs[randnum].id
//     const randMemeName= this.state.allmemeImgs[randnum].title
//     this.setState({
...
//   }
  handleSubmit = event => {
    ...
    const memeCopy = [...this.state.allmemeImgs]
    memeCopy.push(formData)
    this.setState({
    allmemeImgs:memeCopy,
      formData:{
        id:'',
        title:'',
        image:''
...
deleteMeme = (id) =>{
    let memeCopy = [...this.state.allmemeImgs];
    memeCopy = memeCopy.filter( (meme) => (meme.id !== id))
    this.setState({
        allmemeImgs:memeCopy
    })
   }
...
componentDidMount() {
    ...
    axios.get(`https://api.imgflip.com/get_memes`)
        .then(response => {
            const memeImgs = response.data
            const allmemeImgs = memeImgs.data.memes
            this.setState({allmemeImgs})//same as ({persons: persons})
        })
    }
  ...
  render() { 
    let isContainer = this.state.theme ? "container" : "none"
      return (

        <div className={`${isContainer}`} >
          <form className="meme-form" >
          <input type="text" name="topText" placeholder="add top text"
          onChange={this.handleTypingChange} value={this.state.topText}></input>

          <input type="text" name="bottomText"  placeholder="add bottom text"
          onChange={this.handleTypingChange} value={this.state.bottomText}></input>
          
          </form>

          <form className="meme-form" onSubmit={this.handleSubmit}>
          <input type="text" name="title" placeholder="add meme name"
          onChange={this.handleChange} value={this.state.formData.title}></input>

          <input type="text" name="image"  placeholder="add meme url"
          onChange={this.handleChange} value={this.state.formData.image}></input>
          <button type="submit">save</button>
          </form>

          <div className="meme">
                {this.state.allmemeImgs.map((meme) => {
                   return <Meme title={meme.name} img={meme.url} delete={this.deleteMeme} id={meme.id}></Meme>
                })}
          ...
          <button onClick={this.deleteMemes}>Delete all memes</button>
      </div>
      );
  }
}
