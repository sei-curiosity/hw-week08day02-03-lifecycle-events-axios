...
class Memegen extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({allMemeImgs: memeList.map((meme)=>{
            return(
                <div className="meme" id={meme.id} onClick={this.showImage}>
                <h2 className="top">{meme.title}</h2>
                <img src={meme.image} width="600px" alt="" style={{display: this.state.display}}/>
                </div>
            )})})
        }
    ...
    handleSubmit = (event) =>{
        ...
        const randMeme = this.state.allMemeImgs[randNum].image
