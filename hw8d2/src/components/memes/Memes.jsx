...
class Memes extends React.Component{
    ...
    componentDidMount(){
        const allMemes = [
          ...
          this.setState(({...preState})=>{
            preState.allMemes =allMemes
              return preState
          })
    }
    handleMemeTitleClick = () => {
            this.setState(({...preState})=>{
    ...
    render(){
        const imgStatus = this.state.imgStatus ? "show" : "hide"
        return(
            <div>
                <form>
                    <input type='text' name='title' />
                    <input type='text' name='image' />
                    <button>Save Memes</button>
                </form>
                 {this.state.allMemes.map(meme => { return (
                    
                    <div>
                     <h1 key={meme.id} onClick={this.handleMemeTitleClick}>{meme.title}</h1>
                     <div className={imgStatus} id={meme.id}>
